...
namespace NewBrokerApp.Views.Popup
{
	...
	public partial class GetAddress : PopupPage
	{
        ...
        TaskCompletionSource<AdvertisementModel> ChooseCompletionTask;
        AdvertisementModel model;
        public GetAddress(AdvertisementModel data, TaskCompletionSource<AdvertisementModel> ChooseCompletionTask)
        {
            InitializeComponent();
            ...
            else
            {
                Task.Run(async () =>
                {
                    try
                    {
                        ...
                        map.Pins.Add(currentPin);
                        model.Latitude = positionMapnew.Latitude.ToString();
                        model.Longitude = positionMapnew.Longitude.ToString();
                    }
                    catch (Exception ex)
                    {
        ...
        private void map_MapClicked(object sender, MapClickedEventArgs e)
        {
         ...
         //   position.Latitude = pos.Latitude;
           // position.Longitude = pos.Longitude;
            map.MoveToRegion(MapSpan.FromCenterAndRadius(new Xamarin.Forms.GoogleMaps.Position(pos.Latitude, pos.Longitude), Distance.FromKilometers(2)).WithZoom(3));
            map.Pins.Clear();
            currentPin = new Pin
            {
                Label = "",
            ...
            map.Pins.Add(currentPin);
            model.Longitude = pos.Longitude.ToString();
            model.Latitude = pos.Latitude.ToString();
                ...
                Task.Run(async () =>
                {
                    try
                    {
                        ...
                        if (postions != null && postions.Count() != 0)
                        {
                            ...
                            {
                                model.Longitude = Positionaddress.Longitude.ToString();
                                model.Latitude = Positionaddress.Latitude.ToString();
